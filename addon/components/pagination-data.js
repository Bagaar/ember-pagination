...
export default Component.extend({
  ...
  activeItems: computed('_currentPage', 'totalItems', 'itemsPerPage', function () {
  ...
  pageRangePages: computed(
    ...
    function () {
      ...
      let pageRangeOffset = Math.floor(pageRange / 2);
  ...
  startMarginPages: computed('pageMargins', 'pageMarginsThreshold', 'pageRange', 'totalPages', function () {
    if (!this.pageRange) {
      return null;
    }
  ...
  endMarginPages: computed('pageMargins', 'pageMarginsThreshold', 'pageRange', 'pageRangeUpperLimit', 'totalPages', function () {
    if (!this.pageRange) {
      return null;
    }
  ...
  shouldShowUpperBreak: computed('pageRange', 'pageRangePages', 'pageRangeUpperLimit', function () {
    ...
    let pageRangePages = this.pageRangePages;
  ...
  init() {
    ...
    let pageRange = this.pageRange;
    let pageMargins = this.pageMargins;

    assert('@currentPage is required and should be a number', isNumber(this.currentPage));
    assert('@totalItems is required and should be a number', isNumber(this.totalItems));
    assert('@itemsPerPage is required and should be a number', !itemsPerPage || isNumber(itemsPerPage));
    assert(
