...
module('Unit | Component | pagination-data', function (hooks) {
  ...
  test(`it determines the correct 'previousPage' value`, function (assert) {
    ...
    assert.equal(component.previousPage, currentPage - 1);
  });
  ...
  test('previousPage is null when the current page is the first page', function (assert) {
    ...
    assert.equal(component.previousPage, null);
  });
  ...
  test(`it determines the correct 'nextPage' value`, function (assert) {
    ...
    assert.equal(component.nextPage, currentPage + 1);
  });
  ...
  test('nextPage is null when the current page is the last page', function (assert) {
    ...
    assert.equal(component.nextPage, null);
  ...
  test('it yields the total amount of pages', function (assert) {
    ...
    assert.equal(component.totalPages, 6);

    component.set('totalItems', 10);

    assert.equal(component.totalPages, 1);

    component.set('itemsPerPage', 2);

    assert.equal(component.totalPages, 5);
  });
  ...
  test('it yields the amount of items on the current page', function (assert) {
    ...
    assert.equal(component.activeItems, 5);
    ...
    assert.equal(component.activeItems, 10);
    ...
    assert.equal(component.activeItems, 3);
    ...
    assert.equal(component.activeItems, 0);
  });
  ...
  test('it yields an array of all pages as allPages', function (assert) {
    ...
    assert.equal(component.allPages.length, 3);
  ...
  test(
    'pageRangePages contains all pages if the sum of margin pages and the pageRange is larger than the total amount of pages',
    function (assert) {
      let component = this.owner.factoryFor('component:pagination-data').create({
        itemsPerPage: 10,
        totalItems: 50,
      ...
      assert.equal(component.startMarginPages.length, 0);
      assert.equal(component.endMarginPages.length, 0);
      ...
      assert.equal(component.startMarginPages.length, 2);
      assert.equal(component.endMarginPages.length, 2);
